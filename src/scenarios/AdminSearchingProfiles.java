...
import static database.Database.getProfiles;
    ...
    public void update(){
        if (leftClick() == PRESSED){
            if (goBack.isMouseOn()){
                ...
                for (int i = 0; i < profileText.length; i++){
                    if (profileText[i].isMouseOn()){
                        deRenderS();
                        setScenario(new AdminViewProfile(i));
                        break;
                    }
                }
            }
